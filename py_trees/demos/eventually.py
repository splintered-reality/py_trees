...
.. argparse::
   ...
   :prog: py-trees-demo-eventually-program

.. graphviz:: dot/demo-eventually.dot

.. image:: images/demo-eventually.png
...
def description(root: py_trees.behaviour.Behaviour) -> str:
    ...
    """
    content = "The single tick version of the 'eventually' idiom.\n\n"
    ...
    if py_trees.console.has_colours:
        ...
        s = banner_line
        s += console.bold_white + "Eventually".center(79) + "\n" + console.reset
        s += banner_line
        s += "\n"
        s += content
        s += "\n"
...
def create_root(
    ...
    Returns:
        the root behaviour
    """
    set_counting_flag_true = py_trees.behaviours.SetBlackboardVariable(
        name="SetCountingFlagTrue",
        variable_name="counting",
        variable_value=True,
        overwrite=True,
    )
    set_counting_flag_false = py_trees.behaviours.SetBlackboardVariable(
        name="SetCountingFlagFalse",
        variable_name="counting",
        variable_value=False,
        overwrite=True,
    )
    counter = py_trees.behaviours.TickCounter(
        name="Counter", duration=1, completion_status=expected_work_termination_result
    )
    counting = py_trees.composites.Sequence(
        name="Counting", memory=True, children=[set_counting_flag_true, counter]
    )
    root = py_trees.idioms.eventually(
        name="Count and Record",
        worker=counting,
        on_completion=set_counting_flag_false,
    )
    return root
