...
def test_parallel_success_on_selected() -> None:
    ...
    root = py_trees.composites.Parallel(
        name="Parallel",
        policy=py_trees.common.ParallelPolicy.SuccessOnSelected(children=[success1, success2]),
    )
...
def test_parallel_success_on_selected_invalid_configuration() -> None:
    ...
    different_policy = py_trees.common.ParallelPolicy.SuccessOnSelected(children=[running1, running2])
...
def test_parallel_synchronisation() -> None:
    ...
    py_trees.tests.tick_tree(root, 1, 1, visitors=[debug_visitor, snapshot_visitor], print_snapshot=True)
    ...
    py_trees.tests.tick_tree(root, 2, 2, visitors=[debug_visitor, snapshot_visitor], print_snapshot=True)
    ...
    snapshot_visitor.initialise()
    py_trees.tests.tick_tree(root, 3, 3, visitors=[debug_visitor, snapshot_visitor], print_snapshot=True)
