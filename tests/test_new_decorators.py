...
def test_failure_is_success_tree():
    ...
    goon = py_trees.decorators.FailureIsSuccess(py_trees.behaviours.Failure(), "DontBeAfraidToBeTheGoon")
...
def test_inverter_tree():
    ...
    failure = py_trees.behaviours.Failure("Failure")
    failure2 = py_trees.decorators.Inverter(py_trees.behaviours.Success(), "Failure2")
    success = py_trees.behaviours.Success("Success")
    success2 = py_trees.decorators.Inverter(py_trees.behaviours.Failure(), "Success2")
...
def test_running_is_failure_tree():
    ...
    print(console.bold + "****************************************************************************************\n" + console.reset)
    root = py_trees.Selector("Root")
    running = py_trees.decorators.RunningIsFailure(py_trees.behaviours.Running(), "Running")
    failure = py_trees.decorators.RunningIsFailure(py_trees.behaviours.Failure(), "Failure")
    success = py_trees.decorators.RunningIsFailure(py_trees.behaviours.Success(), "Success")
